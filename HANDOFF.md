...
yamuna.html                   YAMUNA (nicht mehr "YAMUNA EPK"): Album, Beschreibung,
                               Pressefotos, Videos, dann Downloads (in dieser Reihenfolge)
...
- Frontend (`assets/js/dates.js`): jeder Termin ein `<details>`-Element,
  antippen/klicken zeigt Zeit (falls nicht `allDay`), Ort (Zeilenumbrüche
  erhalten) und Link (falls vorhanden). Bewusst kein Hover-only-Pattern,
  damit es auf dem Handy genauso funktioniert wie am Desktop.
...
- Kein offener Task-Rückstand aus dem ursprünglichen Umbau-Plan; alles unten
  in "Erledigt" ist live. Neue Wünsche einfach hier oben ergänzen, sobald sie
  reinkommen, und nach Erledigung nach unten verschieben.
